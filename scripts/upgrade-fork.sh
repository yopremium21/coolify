...
CDN="https://raw.githubusercontent.com/yopremium21/coolify/main"
...
HELPER_IMAGE="${HELPER_IMAGE:-ghcr.io/yopremium21/coolify-helper}:${LATEST_HELPER_VERSION}"
...
nohup bash -c "
    ...
    docker run -v /data/coolify/source:/data/coolify/source -v /var/run/docker.sock:/var/run/docker.sock \${DOCKER_CONFIG_MOUNT} --rm \${HELPER_IMAGE} bash -c \"LATEST_IMAGE=\${LATEST_IMAGE} docker compose --env-file /data/coolify/source/.env \${COMPOSE_FILES} up -d --remove-orphans --wait --wait-timeout 60\" >>\"\$LOGFILE\" 2>&1
